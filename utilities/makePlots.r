...
if ( length(args) == 5 ){
    ...
    index = which(! paste(ref.full$CHROM, ref.full$POS) %in% paste(exclude$CHROM, exclude$POS))
    ref = ref.full[index,3]
    alt = alt.full[index,3]
    plaf = read.table(plafFile, header = T)[index,3]
} else {
    ref = ref.full[,3]
    alt = alt.full[,3]
